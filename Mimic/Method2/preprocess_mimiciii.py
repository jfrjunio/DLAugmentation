...
from mimic_augmentation_local import makeDataAugmentation
...
def get_ICD9s_from_mimic_file(fileName, hadmToMap, originalICD9_Codes = None):
	...
	for line in mimicFile:			 #   0  ,     1    ,    2   ,   3  ,    4
		...
		if not ARGS.map_ICD9_to_CCS:
			# Save the original code for each code
			if originalICD9_Codes is not None:
				originalICD9_Codes[ICD9_code[:4]] = ICD9_code
...
def parse_arguments():
	...
	parser.add_argument('proportion', type=int, default=0, help ='Range of proportion (by default 0).')
...
if __name__ == '__main__':
	...
	if len(ARGS.diagnoses_file) > 0:
		...
		get_ICD9s_from_mimic_file(ARGS.diagnoses_file, hadmToICD9CODEs_Map, originalDiagnosisICD9_Codes)
	...
	reverse_CCS_ordered_internalCodesMap = {}
	for i, key in enumerate(CODES_distributionMAP):
		CCS_ordered_internalCodesMap[key[0]]    = i
		# Save the reverse of the mapcode
		reverse_CCS_ordered_internalCodesMap[i] = key[0]
	...
	print '   Success, file length: ' + str(index_of_last_patient_to_dump - index_of_first_patient_to_dump)
	...
	new_all_subjectsListOfCODEsList_LIST = makeDataAugmentation(proportion, new_all_subjectsListOfCODEsList_LIST, originalDiagnosisICD9_Codes, CCS_ordered_internalCodesMap, reverse_CCS_ordered_internalCodesMap)
